...
public class Lfu extends AlgoritmoDeGerencia {
	...
	public int retorne(HashMap<String, TreeMap<Integer,Integer>> frequencia, HashMap<String, TreeMap<Integer,Integer>> tempoEntrada )
	{
		TreeMap<Integer,Integer> t= new TreeMap<>();
		ArrayList<Integer> a= new ArrayList<>();
		ArrayList<Integer> freq= new ArrayList<>();
		
		for(String s:frequencia.keySet())
		{
			for(Integer i:frequencia.get(s).keySet())
			{
				freq.add(frequencia.get(s).get(i));
		...
		for(String process: frequencia.keySet())
		{
			...
			for(Integer i:tempoEntrada.get(process).keySet())
			{
				...
				if(frequencia.get(process).get(i).equals(Collections.min(freq)))
				{
					
					t.put(tempoEntrada.get(process).get(i), i);	
					a.add(tempoEntrada.get(process).get(i));
				}
			}
		
		}
		
		return t.get(Collections.min(a));
	}
	
	public double taxaErroGlobal() {
		int totalErros = 0;
		HashMap<String, Queue<Integer>> dadosFila= new HashMap<>();
		ArrayList<Integer> filaPaginas=new ArrayList<>();
		ArrayList<String> filaProcessos= new ArrayList<>();
		...
		else if(entrada.getAlocacao().equals("Proporcional"))
		{
			...
			for(int i=0;i<entrada.getProcessos().size();i++)
			{
				...
				System.out.println(totalQuadros);
				soma+=totalQuadros;
			}
		...
		for(int i=0;i<entrada.getProcessos().size();i++)
		{
			...
			dadosFila.put(entrada.getProcessos().get(i).getNome(), new LinkedList<>());
		}
		...
		for (int i = 0; i < entrada.getSequencia().size(); i++) 
		{
			...
			if(dadosFila.get(nomeProcesso).contains(pagAcessada)==false)
			{
				...
				if(filaPaginas.size()<totalQuadros)
				{
					...
					{
						int pagRemovida=filaPaginas.remove(0);
						String processoRemovido=filaProcessos.remove(0);
						frequencia.get(processoRemovido).remove(pagRemovida);
						tempoEntrada.get(processoRemovido).remove(pagRemovida);
						dadosFila.get(processoRemovido).remove(pagRemovida);
					...
					else
					{
						...
						String processoRemovido=filaProcessos.remove(filaPaginas.indexOf(pagRemovida));
						filaPaginas.remove(filaPaginas.indexOf(pagRemovida));
						frequencia.get(processoRemovido).remove(pagRemovida);
						tempoEntrada.get(processoRemovido).remove(pagRemovida);
						dadosFila.get(processoRemovido).remove(pagRemovida);
			...
			System.out.println(totalErros+" "+entrada.getSequencia().size());
